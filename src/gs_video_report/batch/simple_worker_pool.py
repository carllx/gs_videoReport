...
class WorkerPool:
    ...
    def _check_output_file_exists(self, task: TaskRecord) -> tuple[bool, str]:
        ...
        try:
            if task.output_path:
                ...
                output_name = f"{video_path.stem}_lesson.md"
    ...
    def _process_video_with_timeout(self, task: TaskRecord, template_params: Dict[str, Any], preferred_model: str, timeout: int) -> Any:
        ...
        try:
            ...
            console.print(f"[yellow]⏰ 处理超时，跳过视频: {task.video_path}[/yellow]")
            raise e
        except Exception as e:
    ...
    def _process_task(self, task: TaskRecord) -> TaskRecord:
        ...
        try:
            ...
            if output_file_exists:
                ...
                task.mark_completed(processing_time=0, output_path=output_path, skip_reason="output_exists")
                with self._lock:
            ...
            # 简单保存文件（这里应该用FileWriter，但为了简化暂时直接写入）
